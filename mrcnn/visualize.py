...
def save_image(image, image_name, boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
    ...
    for index, value in enumerate(useful_mask_indices):
        ...
        if mode != 2:
         ...
         # Label
        #path_to_font = os.getcwd() + "\\mechpart\\Arial.ttf"
        #print("Path to font: ", path_to_font)
        # font = ImageFont.truetype('/Library/Fonts/Arial.ttf', 1)
        #font = ImageFont.truetype(path_to_font, size=1)
        font = ImageFont.truetype("arial.ttf", size=1)
        draw.text((x1, y1), "%s %f" % (label, score), (255, 255, 255), font)
    masked_image.save(os.path.join(save_dir, '%s.jpg' % (image_name)))
    return os.path.join(save_dir, '%s.jpg' % (image_name))
